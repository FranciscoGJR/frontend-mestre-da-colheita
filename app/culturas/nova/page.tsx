...
export default function NovaCulturaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetchApi("/culturas", {
        method: "POST",
        body: JSON.stringify(formData),
      })
      router.push("/culturas")
    } catch (err) {
      setError("Erro ao criar cultura. Verifique os dados e tente novamente.")
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-green-700">Nova Cultura</h1>
      </div>
